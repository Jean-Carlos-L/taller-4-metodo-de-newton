...
class NewtonParelela {
  ...
  */
  ...
  val umbral = 1000 // umbral para determinar si se debe paralelizar o no
  ...
  def derivar(f: Expr, a: Atomo, umbral: Int = umbral): Expr = f match {
  ...
  def evaluar(f: Expr, a: Atomo, v: Double, umbral: Int = umbral): Double = f match {
    case Numero(d) => d
